...
class MultimodalDataset_visual(Dataset):
    def __init__(self, dataset_json_file, conf, label_map, face_model, face_size, tokenizer_model):
        """
        Dataset that manages video recordings
        :param conf: Dictionary containing the audio loading and preprocessing settings
    ...
    def __getitem__(self, index):
        ...
        text = datum['text']
        fbank = (fbank - self.norm_mean) / (self.norm_std * 2)
        origi_fbank = copy.deepcopy(fbank)
        ...
        return fbank, images, text, label_indices, ids, origi_fbank
...
def collate_fn_visual(batch):
    ...
    origi_fbanks = []
    for dp in batch:
        fbank, sampledImgs, text, label, curr_id, ori_fbank = dp
        if sampledImgs is None:
            continue
        fbanks.append(fbank)
        imgs.append(sampledImgs)
        ...
        texts.append(text)
        ids.append(curr_id)
        origi_fbanks.append(ori_fbank)
    return torch.stack(fbanks), torch.stack(imgs), texts, torch.stack(labels), ids, origi_fbanks
