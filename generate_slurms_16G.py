...
def output_slurm(dataset, pretrained_type, bz, lr, epoch, seed, face_size, layer_loss_factor, constrastive_loss_factor, slurm_root_path):
    gpu_device = random.choice(['tesla_v100-sxm2-16gb'])
...
if __name__ == '__main__':
    slurm_files = []
    slurm_root_path = 'push_pull_iter_contrastive_loss_progressive_0218/'
    bz = 8
    epoch = 40
    ...
    for dataset in ["mosei", "iemocap"]:
        for layer_loss_factor in [0.25]:
            for constrastive_loss_factor in [0.25]:
                for lr in [1e-4, 1e-5]:
