...
class MTModel(nn.Module):
    def __init__(self, label_dim, video_model, text_model, modals="tv"):
        ...
        self.video_map1 = nn.Linear(768, 768)
    ...
    def forward(self, audio_input, video_input, text_input):
        ...
        text_inte_embedding = self.interaction(text_hidden_states,video_hidden_states,self.text_map1, self.text_map2_text,self.text_map2_audio, self.text_map2_video,self.text_attention, self.layerNorm_text, self.layerNorm_audio, self.layerNorm_video)
        video_inte_embedding = self.interaction(video_hidden_states, text_hidden_states, self.video_map1, self.video_map2_video,self.video_map2_text,self.video_map2_audio,self.video_attention, self.layerNorm_video, self.layerNorm_text, self.layerNorm_audio)
        ...
        multimode_pred = self.fusion(torch.cat((text_inte_embedding, text_cls, video_inte_embedding, video_cls),-1))
        result = self.weighted_fusion(torch.stack((text_pred, video_pred, multimode_pred),-1)).squeeze(-1)
        return result