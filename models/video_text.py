...
class video_text(nn.Module):
    def __init__(self, label_dim, audio_model, video_model, text_model, modals="tav"):
        super(video_text, self).__init__()
        self.text_model = text_model
        self.video_model = video_model
        self.fusion = nn.Linear(768*2, label_dim)
    ...
    def forward(self, audio_input, video_input, text_input):
        ...
        video_cls_embedding = video_last_hidden_state[:, 0, :]
        text_cls_embedding = text_last_hidden_state[:, 0, :]
        result=self.fusion(torch.cat((video_cls_embedding, text_cls_embedding),-1))
        return result